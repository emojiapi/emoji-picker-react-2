...
const cleanEmoji = emoji => {
  ...
  emoji[constants.EMOJI_PROPERTY_NAME] = [
    ...new Set(
  ...
  if (emoji.skin_variations) {
    emoji[constants.EMOJI_PROPERTY_SKIN_VARIATIONS] = Object.values(
      _.mapValues(emoji.skin_variations, 'unified')
    );
  }

  return _.pick(emoji, [
    constants.EMOJI_PROPERTY_NAME,
    constants.EMOJI_PROPERTY_UNIFIED,
    constants.EMOJI_PROPERTY_SKIN_VARIATIONS,
  ]);
};
...
const { groupedEmojis } = emojis.reduce(
  ({ groupedEmojis }, emoji) => {
    ...
    const category = emoji.category.replace(' & ', '_');
...
// fs.writeFileSync(
//   './src/skinTones.json',
//   JSON.stringify(['neutral', '1f3fb', '1f3fc', '1f3fd', '1f3fe', '1f3ff']),
//   'utf8'
// );
